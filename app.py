...
@socketio.on('subscribe_to_detected')
def subscribeToDetected(data):
	# Subscribe to all changes of resource /3300/0/5700 (button presses)
	print('subscribe_to_detected: ',data)
	e = connector.putResourceSubscription(data['endpointName'],'/3300/0/5700')
	while not e.isDone():
		None
	if e.error:
		print("Error: ",e.error.errType, e.error.error, e.raw_data)
...
@socketio.on('unsubscribe_to_detected')
def unsubscribeToDetected(data):
	print('unsubscribe_to_detected: ',data)
	e = connector.deleteResourceSubscription(data['endpointName'],'/3300/0/5700')
	while not e.isDone():
		None
	if e.error:
		print("Error: ",e.error.errType, e.error.error, e.raw_data)
		...
		print("Unsubscribed Successfully!")
	emit('unsubscribed-to-detected',{"endpointName":data['endpointName'],"value":'True'})
...
@socketio.on('get_detected')
def getDetected(data):
	# Read data from GET resource /3300/0/5700 (num button presses)
	print("get_detected ",data)
	e = connector.getResourceValue(data['endpointName'],'/3300/0/5700')
	while not e.isDone():
		None
	if e.error:
		print("Error: ",e.error.errType, e.error.error, e.raw_data)
		...
		data_to_emit = {"endpointName":data['endpointName'],"value":e.result}
		print data_to_emit
		emit('detected', data_to_emit)

@socketio.on('pir_control')
def pirControl(data):
	# Set data on PUT resource /3300/0/5701 (PIR Control)
    print('pir_on ',data)
    
    e = connector.postResource(data['endpointName'],'/3300/0/5701', data['value'])
    while not e.isDone():
        None
    if e.error:
        print("Error: ",e.error.errType, e.error.error, e.raw_data)
